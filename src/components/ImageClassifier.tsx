...
export default function ImageClassifier({ config }: { config: ImageDemoConfig }) {
  ...
  async function run(queue: Photo[]) {
    ...
    setStatus(`${queue.length} photo${queue.length === 1 ? '' : 's'} classified.`);
  ...
  return <div className="image-classifier">
    <div className="ic-toolbar">
      <div><span className="tag">{config.samples.length} real images · {Object.keys(config.labels).length} labels</span></div>
      <div className="ic-actions">
        ...
        {running ? <button className="btn" onClick={() => { cancel(); setStatus('Stopped. Results kept.'); trackDemoEvent(config.slug, 'run_cancelled'); }}>Stop</button>
          : <button className="btn" disabled={reading} onClick={() => void run(config.samples)}>Classify sample set</button>}
      </div>
    </div>
    <p className={`ic-status${error ? ' is-error' : ''}`} role="status" hidden={!status}>{status}</p>
    <div className="ic-workbench">
      ...
      <section className="ic-inspector" aria-label="Selected image classification">
        <div className="ic-annotation"><span className="ic-eyebrow">Dataset annotation</span><strong>{selected.expected ?? 'Your image · no reference label'}</strong></div>
        <div className="ic-result" aria-busy={running === selected.id}>
          ...
          </> : <><h2>{running === selected.id ? 'Reading the image…' : 'What does the model see?'}</h2><p>Classify to see the label scores.</p></>}
        </div>
    ...
    <div className="ic-gallery-heading"><h2>Sample photos</h2><span>{scored.length ? `${matched} of ${scored.length} results match the dataset label` : 'Select an image to inspect it'}</span></div>
    ...
    <details className="panel ic-details"><summary>Use cases & limits</summary><div className="ic-context"><section><h2>Where this helps</h2><p>{config.useCase}</p></section><section><h2>Know the limits</h2><p>{config.limitation}</p><p>This small sample illustrates the workflow; label agreement is not a benchmark.</p></section></div></details>
