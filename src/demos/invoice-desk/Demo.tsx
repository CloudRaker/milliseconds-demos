...
export default function Demo(){
 ...
 return <div className="invoice-desk">
  ...
  <div className="id-workspace">
   <section className="id-source"><div className="id-section-head"><div><p className="id-eyebrow">01 / Source document</p><h2>The invoice</h2></div><span className="muted">{source===invoice(SAMPLES[sample].draft)?'Fictional sample':'Your source text'}</span></div>
   ...
   <section className="id-record"><div className="id-section-head"><div><p className="id-eyebrow">02 / Draft record</p><h2>{ready?'Ready for human review':draft?'Review exceptions':'Awaiting processing'}</h2></div>{ready?<CheckCircleIcon className="id-good" size={25}/>:<WarningCircleIcon size={25} className="id-warning"/>}</div>
  ...
  <section className="id-checks"><div className="id-section-head"><div><p className="id-eyebrow">03 / Review queue</p><h2>{draft?`${issues.length} ${issues.length===1?'item':'items'} to review`:'Process an invoice to see checks'}</h2></div><div className="id-counts"><span><b>{missing.length}</b> missing</span><span><b>{conflicts.length}</b> discrepancies</span><span><b>{uncertain.length}</b> source checks</span></div></div>
